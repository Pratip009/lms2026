...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cd-body {
    max-width: 1280px; margin: 0 auto;
    padding: 28px min(64px, 5vw) 96px;
    display: grid;
    grid-template-columns: 1fr minmax(0, 368px);
  ...
  .cd-curriculum-head-meta { font-size: 12.5px; color: var(--ink-4); }
  ...
  .cd-lesson-num-done { background: var(--green-lt); border-color: var(--green-mid); color: var(--green); }
  ...
  /* ════════ PURCHASE CARD ════════ */
  .cd-body-right { position: sticky; top: 24px; min-width: 0; }
  ...
  .cd-card-thumb {
    display: none; width: 100%; aspect-ratio: 16/9; overflow: hidden; position: relative;
  }
  ...
  .cd-btn-enroll:hover:not(:disabled) {
    background: var(--b600); transform: translateY(-2px);
  ...
  @media (max-width: 1024px) {
    ...
    .cd-body {
      grid-template-columns: 1fr;
      padding: 24px min(40px, 5vw) 64px;
    }
    /* Card goes first on tablet/mobile */
    .cd-body-right {
      order: -1;
      position: static;
    }
    /* Show thumb inside card on tablet */
  ...
  @media (max-width: 640px) {
    .cd-hero-inner,
    .cd-body { padding-left: 16px; padding-right: 16px; }

    .cd-hero-content { padding: 28px 0 28px; }

    .cd-hero-title { font-size: clamp(20px, 6vw, 28px); }

    .cd-hero-sub { font-size: 13px; margin-bottom: 20px; }

    .cd-hero-meta { gap: 12px 16px; font-size: 12px; }

    .cd-learn-grid { grid-template-columns: 1fr; }

    .cd-alert { padding: 12px 16px; }

    .cd-card-body { padding: 18px 18px 20px; }

    .cd-card-price { font-size: 28px; }

    .cd-lesson { padding: 10px 14px; gap: 10px; }

    .cd-lesson-title { font-size: 12.5px; }

    /* Show standalone mobile thumb in left column */
    .cd-mobile-thumb { display: block; }

    /* Hide card thumb on mobile since we show standalone above */
    .cd-card-thumb { display: none; }

    .cd-body-right { order: 0; }

    .cd-section-title { font-size: 17px; }
  }

  /* Small mobile: ≤400px */
  @media (max-width: 400px) {
    .cd-hero-title { font-size: 18px; }
    .cd-card-price { font-size: 24px; }
    .cd-btn-enroll,
    .cd-btn-continue,
    .cd-btn-admin { padding: 13px 16px; font-size: 13px; }
  }
`;
...
export default function CourseDetail() {
  ...
  /* Split title: first half plain, second half gradient */
  const titleWords = course.title?.split(' ') || [];
  const half       = Math.ceil(titleWords.length / 2);
  const titleFirst = titleWords.slice(0, half).join(' ');
  const titleRest  = titleWords.slice(half).join(' ');
  ...
  return (
    <div className="cd">
      ...
      <div className="cd-body">
        ...
        {/* Left column */}
        <div>
          {/* Standalone mobile thumbnail (shown only on small screens) */}
